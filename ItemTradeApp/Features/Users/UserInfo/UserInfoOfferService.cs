...
{
    Task<Result<PagedResponse<OfferListingDTO>>> GetPagedActiveAsync(int id, int page, int pageSize,
        CancellationToken ct = default);

    Task<Result<PagedResponse<OfferListingDTO>>> GetPagedHistoryAsync(int id, int page, int pageSize,
        CancellationToken ct = default);
}
...
{
    public async Task<Result<PagedResponse<OfferListingDTO>>> GetPagedActiveAsync(int id, int page, int pageSize, CancellationToken ct = default)
    {
        ...
        var (userOffers,userOffersTotalCount) = await userInfoOfferRepository.GetActiveForUserByIdPagedAsync(id,page,pageSize,ct);
    ...
    public async Task<Result<PagedResponse<OfferListingDTO>>> GetPagedHistoryAsync(int id, int page, int pageSize, CancellationToken ct = default)
    {
        if (page <= 0) return Result<PagedResponse<OfferListingDTO>>.BadRequest("invalid_page_number");
        if (pageSize <= 0) return Result<PagedResponse<OfferListingDTO>>.BadRequest("invalid_page_size");
        pageSize = pageSize > 100 ? 100 : pageSize;
        var userExists = await userInfoRepository.ExistsByIdAsync(id, ct);
        if (!userExists) return Result<PagedResponse<OfferListingDTO>>.NotFound("user_not_found");
        var (userOffers,userOffersTotalCount) = await userInfoOfferRepository.GetHistoryForUserByIdPagedAsync(id,page,pageSize,ct);
        var totalPages = (int)Math.Ceiling(userOffersTotalCount / (double)pageSize);
